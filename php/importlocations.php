...
$sqlTable = "CREATE TABLE IF NOT EXISTS locations(id INTEGER PRIMARY KEY AUTOINCREMENT,name VARCHAR,abbr VARCHAR,floor VARCHAR,floorplanfn VARCHAR,sortid VARCHAR,nameid VARCHAR)";
...
<br />
<br />
<a href='../data/files/locations_import_template.csv'><img src='../images/xcel2.jpg' height=25 border=0></a> <a href='../data/files/locations_import_template.csv'>Template</a>
<br />
<br />
</p>
</center>
...
?>
    ...
    <tr>
    <?php
        echo "<th>ID</th><th>Name</th><th>Abbreviation</th><th>Floor</th><th>Floor Plan Filename</th><th>Sort ID</th><th>Name ID</th>";
   ?>
    </tr>
	</thead>
	<tbody>
...
if ($nextstep==2) {
	...
	// Add locations Records
	foreach ($imlines as $line_num => $locations) {
		if ($line_num==0 && $_POST['skip1st']) {
			echo "<br>Skipped the first line (header)<br>";
			echo "<br><h2>Locations Data Imported Successfully</h2><br>";	
			continue;
		}

		$cols=explode($delim,$locations);

		$id=$cols[$name2fno['id']];
		$name=$cols[$name2fno['name']];
		$abbr=$cols[$name2fno['abbr']];
		$floor=$cols[$name2fno['floor']];
		$floorplanfn=$cols[$name2fno['floorplanfn']];
		$sortid=$cols[$name2fno['sortid']];
		$nameid=$cols[$name2fno['nameid']];

	$sql=	"INSERT into locations ".
			"(id,name,abbr,floor,floorplanfn,sortid,nameid) ".
			" VALUES ".
			"(:id,:name,:abbr,:floor,:floorplanfn,:sortid,:nameid)";
		...
		echo "<center><br><h2>Locations Data Imported Successfully</h2><br></center>";	
}
