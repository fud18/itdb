...
if ($export) 
  ...
  echo "<h1>Building Name Here <a title='Add new jack' href='../../index.php?action=editjack&amp;id=new'>".
...
<!-- Location Information -->
		<td><center><?php te("Building Initials Here");?></center></td>
...
$sth=db_execute($dbh,"SELECT count(jacks.id) as totalrows, locations.nameid FROM jacks JOIN locations WHERE locations.id = locationid AND nameid = 00000 $where");
...
$t=time();
$sql="SELECT jacks.*, locations.nameid FROM jacks JOIN locations WHERE locations.id = locationid AND nameid = 00000 $where order by $orderby LIMIT $totalrows";
